...
module.exports = class extends Monitor {
	...
	async addRole(message, level) {
		let guildSettings = await this.client.providers.default.get('guildSettings', message.guild.id);
		if (!guildSettings || !guildSettings.level || !guildSettings.level[level.toString()]) return;
		let role = message.guild.roles.cache.get(guildSettings.level[level.toString()]);
		message.member.addRole(role).catch(e => this.emit('error', e));
	}
